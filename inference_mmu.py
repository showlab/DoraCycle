...
from training.prompting_utils import UniversalPrompting, create_attention_mask_for_mmu, create_attention_mask_for_mmu_vit, create_attention_mask_for_mmu_perturbed
    ...
    for i, file_name in enumerate(tqdm(file_list)):
        ...
        for question in config.question:
            if config.model.showo.w_clip_vit:
                ...
                attention_mask, attention_mask_perturbed = create_attention_mask_for_mmu_perturbed(input_ids.to(device), eoi_id=int(uni_prompting.sptids_dict['<|eoi|>']), mask_rate=0.90)

                # cont_toks_list = model.mmu_generate(input_ids, attention_mask=attention_mask,
                #                             max_new_tokens=256, top_k=top_k,
                #                             eot_token=uni_prompting.sptids_dict['<|eot|>'])
                
                cont_toks_list = model.mmu_generate_mask(input_ids, attention_mask=attention_mask, attention_mask_perturbed= attention_mask_perturbed,
                                            max_new_tokens=256, top_k=top_k,
                                            eot_token=uni_prompting.sptids_dict['<|eot|>'], text_cfg=1.0)
