...
"""
Multimodal understanding (e.g. captioning) inference with LoRA-loaded DoraCycle checkpoint.
...
Set config.mmu_image_root to the directory of images to caption.
"""
...
from training.prompting_utils import (
    ...
    create_attention_mask_for_mmu_perturbed,
)
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(ckpt_path):
        raise FileNotFoundError(
            f"Checkpoint not found: {ckpt_path}. Set pretrained_model_path to your DoraCycle checkpoint directory."
        )
    ...
    for i, file_name in enumerate(tqdm(file_list)):
        ...
        for question in config.question:
            if config.model.showo.w_clip_vit:
                ...
                attention_mask, attention_mask_perturbed = create_attention_mask_for_mmu_perturbed(
                    input_ids.to(device),
                    eoi_id=int(uni_prompting.sptids_dict["<|eoi|>"]),
                    mask_rate=0.90,
                )
                cont_toks_list = model.mmu_generate_mask(
                    input_ids,
                    attention_mask=attention_mask,
                    attention_mask_perturbed=attention_mask_perturbed,
                    max_new_tokens=256,
                    top_k=top_k,
