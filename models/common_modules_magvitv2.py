...
class Conv3d_upT(nn.Conv3d):

    def __init__(self, chan_in, chan_out, *args, **kwargs):
        self.channel_to_time = kwargs.pop("channel_to_time", False)
        if self.channel_to_time:
            out_cmul = 2
        else:
            out_cmul = 1
        super().__init__(chan_in, chan_out * out_cmul, *args, **kwargs)

    def forward(self, x):

        x = super().forward(x)
        if self.channel_to_time:
            x = x.reshape(x.shape[0], 2, x.shape[1]//2, x.shape[2], x.shape[3], x.shape[4])
            x = rearrange(x, "b k c t h w -> b c (t k) h w")
...
class TimeDownsample2x(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.reshape(out, [b, h, w, c, -1])
...
class AttnBlock_Multiview(nn.Module):
    ...
    def forward(self, x, kv):
        ...
        # k = k.reshape(b, c, h * w)  # b,c,2hw
        w_ = torch.bmm(q, k)  # b,hw,2hw    w[b,i,j]=sum_c q[b,i,c]k[b,c,j]
        w_ = w_ * (int(c) ** (-0.5))
        w_ = torch.nn.functional.softmax(w_, dim=2)

        # attend to values
        # v = v.reshape(b, c, h * w)
        w_ = w_.permute(0, 2, 1)  # b,2hw,hw (first hw of k, second of q)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if padding is None:
            ...
            time_pad, height_pad, width_pad = padding

        self.time_pad = time_pad
        self.conv = nn.Conv3d(chan_in, chan_out, kernel_size, stride=stride, padding=(0, height_pad, width_pad))

    def forward(self, x):
        if self.time_pad != 0:
            first_frame_pad = x[:, :, :1, : ,:].repeat((1, 1, self.time_pad * 2, 1, 1))
