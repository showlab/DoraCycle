...
def remove_prefix(caption):
    caption = caption.replace('The image features ', '').replace('The image presents ', '').replace(
        ...
        "This image captures ", '').replace("This image showcases", '')
...
class Text2ImageDataset:
    def __init__(
            ...
            caption_prompt_path: Optional[str] = None,
        ...
        if self.add_caption_prompt:
            _path = caption_prompt_path or os.environ.get("CAPTION_PROMPT_JSON", "")
            if not _path or _path.startswith("<"):
                raise ValueError("add_caption_prompt=True requires caption_prompt_path or env CAPTION_PROMPT_JSON")
        ...
        num_batches = num_worker_batches * num_workers  # num_workers = dataset_config.num_workers = 32
        num_samples = num_batches * global_batch_size  # global_batch_size = 16 = 2 (# batch size in on gpu)* 8 (# gpus)
